...
class BaseModel(nn.Module):
    def __init__(self, heads, head_convs, num_stacks, last_channel, opt=None):
        ...
        if opt.trades:
            ...
            self.attention_cur = nn.Conv2d(64, 1, kernel_size=(opt.deform_kernel_size, opt.deform_kernel_size),
                                           stride=1, dilation=(1, 1), padding=(1, 1), bias=True)
            self.attention_prev = nn.Conv2d(64, 1, kernel_size=(opt.deform_kernel_size, opt.deform_kernel_size),
                                            stride=1, dilation=(1, 1), padding=(1, 1), bias=True)
            self.conv_offset_w = nn.Conv2d(129, opt.deform_kernel_size * opt.deform_kernel_size,
                                           kernel_size=(opt.deform_kernel_size, opt.deform_kernel_size),
                                           stride=1, dilation=(1, 1), padding=(1, 1), bias=True)
            self.conv_offset_h = nn.Conv2d(129, opt.deform_kernel_size * opt.deform_kernel_size,
                                           kernel_size=(opt.deform_kernel_size, opt.deform_kernel_size),
                                           stride=1, dilation=(1, 1), padding=(1, 1), bias=True)
    ...
    def forward(self, x, pre_img=None, pre_hm=None, addtional_pre_imgs=None, addtional_pre_hms=None,
                inference_feats=None, inference_feats_shallow=None, inference_feats_middle=None):
        cur_feat, cur_feat_shallow, cur_feat_middle = self.img2feats(x)

        assert self.num_stacks == 1
        if self.opt.trades:
            feats, embedding, tracking_offset, dis_volume, h_volume_aux, w_volume_aux \
                = self.TraDeS(cur_feat, cur_feat_shallow, cur_feat_middle, pre_img, pre_hm, addtional_pre_imgs,
                              addtional_pre_hms,
                              inference_feats, inference_feats_shallow, inference_feats_middle)
        else:
            feats = [cur_feat[0]]
        ...
        if self.opt.inference:
            return out, cur_feat[0].detach().cpu().numpy(), cur_feat_shallow[0].detach().cpu().numpy(), cur_feat_middle[
                0].detach().cpu().numpy()
        else:
            return out

    def TraDeS(self, cur_feat, cur_feat_shallow, cur_feat_middle, pre_img, pre_hm, addtional_pre_imgs,
               addtional_pre_hms,
               inference_feats, inference_feats_shallow, inference_feats_middle):
        feat_list = []
        feat_shallow_list = []
        ...
        feat_shallow_list.append(cur_feat_shallow[0])
        feat_middle_list.append(cur_feat_middle[0])
        support_feats = []
        if self.opt.inference:
            for prev_feat in inference_feats:
                feat_list.append(torch.from_numpy(prev_feat).to(self.opt.device)[:, :, :, :])
            for prev_feat_shallow in inference_feats_shallow:
                feat_shallow_list.append(torch.from_numpy(prev_feat_shallow).to(self.opt.device)[:, :, :, :])
            for prev_feat_middle in inference_feats_middle:
                feat_middle_list.append(torch.from_numpy(prev_feat_middle).to(self.opt.device)[:, :, :, :])
            while len(feat_list) < self.opt.clip_len:  # only operate in the initial frame
                feat_list.append(cur_feat[0])
            while len(feat_shallow_list) < self.opt.clip_len:
                feat_shallow_list.append(cur_feat_shallow[0])
            while len(feat_middle_list) < self.opt.clip_len:
                feat_middle_list.append(cur_feat_middle[0])
            ...
            feat2, feat2_shallow, feat2_middle = self.img2feats_prev(pre_img)
            pre_hm_1 = self.avgpool_stride4(pre_hm)
            feat_list.append(feat2[0])
            feat_shallow_list.append(feat2_shallow[0])
            feat_middle_list.append(feat2_middle[0])
            support_feats.append(feat2[0] * pre_hm_1)
            for ff in range(len(addtional_pre_imgs) - 1):
                feats_ff, feats_ff_shallow, feats_ff_middle = self.img2feats_prev(addtional_pre_imgs[ff])
                pre_hm_i = self.avgpool_stride4(addtional_pre_hms[ff])
                feat_list.append(feats_ff[0][:, :, :, :])
                feat_shallow_list.append(feats_ff_shallow[0][:, :, :, :])
                feat_middle_list.append(feats_ff_middle[0][:, :, :, :])
                support_feats.append(feats_ff[0][:, :, :, :] * pre_hm_i)

        return self.CVA_MFW(feat_list, feat_shallow_list, feat_middle_list, support_feats)

    def CVA_MFW(self, feat_list, feat_shallow_list, feat_middle_list, support_feats):
        prop_feats = []
        attentions = []
        h_max_for_loss_aux = []
        w_max_for_loss_aux = []
        feat_cur = feat_list[0]
        feat_cur_shallow = feat_shallow_list[0]
        feat_cur_middle = feat_middle_list[0]
        ...
        embedding_shallow = self.embedconv(feat_cur_shallow)
        embedding_middle = self.embedconv(feat_cur_middle)
        embedding_prime = self.maxpool_stride2(embedding)
        embedding_prime_shallow = self.maxpool_stride2(embedding_shallow)
        embedding_prime_middle = self.maxpool_stride2(embedding_middle)
        # (B, 128, H, W) -> (B, H*W, 128):
        embedding_prime = embedding_prime.view(batch_size, self.embed_dim, -1).permute(0, 2, 1)
        embedding_prime_shallow = embedding_prime_shallow.view(batch_size, self.embed_dim, -1).permute(0, 2, 1)
        embedding_prime_middle = embedding_prime_middle.view(batch_size, self.embed_dim, -1).permute(0, 2, 1)
        attention_cur = self.attention_cur(feat_cur)
        attentions.append(attention_cur)
        for idx, feat_prev in enumerate(feat_list[1:]):
            c_h, c_w, tracking_offset = self.CVA(embedding_prime, embedding_prime_shallow, embedding_prime_middle,
                                                 feat_prev, feat_shallow_list[idx], feat_middle_list[idx],
                                                 batch_size, h_c, w_c)
        ...
        return [enhanced_feat], embedding, tracking_offset_output, [h_max_for_loss,
                                                                    w_max_for_loss], h_max_for_loss_aux, w_max_for_loss_aux

    def CVA(self, embedding_prime, embedding_prime_shallow, embedding_prime_middle,
            feat_prev, feat_prev_shallow, feat_prev_middle,
            batch_size, h_c, w_c):
        embedding_prev = self.embedconv(feat_prev)
        embedding_prev_shallow = self.embedconv(feat_prev_shallow)
        embedding_prev_middle = self.embedconv(feat_prev_middle)
        _embedding_prev = self.maxpool_stride2(embedding_prev)
        _embedding_prev_shallow = self.maxpool_stride2(embedding_prev_shallow)
        _embedding_prev_middle = self.maxpool_stride2(embedding_prev_middle)
        _embedding_prev = _embedding_prev.view(batch_size, self.embed_dim, -1)
        _embedding_prev_shallow = _embedding_prev_shallow.view(batch_size, self.embed_dim, -1)
        _embedding_prev_middle = _embedding_prev_middle.view(batch_size, self.embed_dim, -1)

        c = torch.matmul(embedding_prime, _embedding_prev)  # (B, H*W/4, H*W/4)
        c_shallow = torch.matmul(embedding_prime_shallow, _embedding_prev_shallow)
        c_middle = torch.matmul(embedding_prime_middle, _embedding_prev_middle)
        c_shallow = F.interpolate(c_shallow.unsqueeze(0), size=(c.shape[1], c.shape[2]), mode='bilinear',
                                  align_corners=False).squeeze(0)
        c_middle = F.interpolate(c_middle.unsqueeze(0), size=(c.shape[1], c.shape[2]), mode='bilinear',
                                 align_corners=False).squeeze(0)
        c = c.view(batch_size, h_c * w_c, h_c, w_c)  # (B, H*W, H, W)
        c_shallow = c_shallow.view(batch_size, h_c * w_c, h_c, w_c)
        c_middle = c_middle.view(batch_size, h_c * w_c, h_c, w_c)
        c = c + c_shallow + c_middle
