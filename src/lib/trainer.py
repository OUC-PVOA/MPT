...
class Trainer(object):
  ...
  def run_epoch(self, phase, epoch, data_loader):
    ...
    for iter_id, batch in enumerate(data_loader):
      ...
      if processed_images >= 100:
        print(f'Epoch: {epoch}, Iteration: {iter_id}, Loss: {loss.item()}')
        processed_images = 0
  ...
  def debug(self, batch, output, iter_id, dataset):
    ...
    for i in range(1):
      ...
      for k in range(len(dets[i]['scores'])):
        for score in dets[i]['scores'][k]:
          if score.item() > opt.vis_thresh:
            debugger.add_coco_bbox(
              dets[i]['bboxes'][k] * opt.down_ratio, dets[i]['clses'][k],
              dets[i]['scores'][k], img_id='out_pred')

            if 'ltrb_amodal' in opt.heads:
              debugger.add_coco_bbox(
                dets[i]['bboxes_amodal'][k] * opt.down_ratio, dets[i]['clses'][k],
                dets[i]['scores'][k], img_id='out_pred_amodal')

            if 'hps' in opt.heads and int(dets[i]['clses'][k]) == 0:
              debugger.add_coco_hp(
                dets[i]['hps'][k] * opt.down_ratio, img_id='out_pred')

            if 'tracking' in opt.heads:
              debugger.add_arrow(
                dets[i]['cts'][k] * opt.down_ratio,
                dets[i]['tracking'][k] * opt.down_ratio, img_id='out_pred')
              debugger.add_arrow(
                dets[i]['cts'][k] * opt.down_ratio,
                dets[i]['tracking'][k] * opt.down_ratio, img_id='pre_img_pred')

      # Ground truth
      debugger.add_img(img, img_id='out_gt')
      debugger.add_img(img, img_id='pre_img_gt')
