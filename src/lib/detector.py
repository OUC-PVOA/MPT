...
class Detector(object):
    def __init__(self, opt):
        ...
        self.inference_feats_shallow = deque(maxlen=self.opt.clip_len - 1)
        self.inference_feats_middle = deque(maxlen=self.opt.clip_len - 1)
    ...
    def process(self, images, pre_images=None, pre_hms=None, return_time=False):
        with torch.no_grad():
            ...
            if self.opt.trades:
                ...
                output, current_feat, current_feat_shallow, current_feat_middle = self.model(
                    images, pre_images, pre_hms, inference_feats=self.inference_feats,
                    inference_feats_shallow=self.inference_feats_shallow,
                    inference_feats_middle=self.inference_feats_middle,
                ...
                self.inference_feats_shallow.appendleft(current_feat_shallow)
                self.inference_feats_middle.appendleft(current_feat_middle)
                while len(self.inference_feats) < (self.opt.clip_len - 1):
                    self.inference_feats.appendleft(current_feat)
                while len(self.inference_feats_shallow) < (self.opt.clip_len - 1):
                    self.inference_feats_shallow.appendleft(current_feat_shallow)
                while len(self.inference_feats_middle) < (self.opt.clip_len - 1):
                    self.inference_feats_middle.appendleft(current_feat_middle)
            else:
